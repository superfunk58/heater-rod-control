...
// ----- Konfiguration -----
static constexpr uint32_t REQUEST_INTERVAL_MS = 3000;   // alle 3s ein neuer Read
...
static bool readSensorByRom(uint64_t rom, float &out) {
  if (!s_dt || s_count == 0) return false;
  uint8_t addr[8];
  romToBytes(rom, addr);
  float t = s_dt->getTempC(addr);
  if (t == DEVICE_DISCONNECTED_C || t < T_MIN_VALID || t > T_MAX_VALID) return false;
  out = t;
  return true;
}
...
void tick() {
  ...
  for (uint8_t i = 0; i < s_count && i < MAX_SENSORS; i++) {
    float t;
    const bool ok = readSensorByRom(s_roms[i], t);
    if (ok) {
      s_vals[i] = t;
      if (s_failStreak[i] > 0) {
      ...
      if (s_failStreak[i] == 1) {
        webLog("[Temp] Sensor %d FAIL (streak=%d)", i, s_failStreak[i]);
      }
    }
  }

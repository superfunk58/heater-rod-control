...
// Long-term: 672 samples / 7 chunks = 96 samples per chunk
...
static constexpr uint32_t SCHEMA_VERSION = 10;  // bumped: retention 2d short / 7d long
...
void begin() {
  ...
  if (schema != SCHEMA_VERSION) {
    Serial.printf("[HIST] schema %u != %u, migrating...\n", schema, SCHEMA_VERSION);
    // Do NOT discard data: the 20-byte Sample structure has been stable.
    // We validate individual samples below instead.
  }
  ...
  if (cnt_long <= CAPACITY_LONG && idx_long < CAPACITY_LONG) {
...
void tickSample(int powerdraw, int wattneeded, int solarpower,
  ...
  time_t now;
  time(&now);
  if (now < 1700000000) return;   // NTP not synced
